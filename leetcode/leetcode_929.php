...
//there will be forwarded to the same address without dots in the local name. Note that this rule does not
...
function numUniqueEmails($emails) {
    ...
    for ($i = 0; $i< count($emails); $i++) {
        ...
        $mailEmail = $local.'@'.$domain;
        if(!in_array($mailEmail, $final_array)) {
            $final_array[] = $mailEmail;
        }
    }
